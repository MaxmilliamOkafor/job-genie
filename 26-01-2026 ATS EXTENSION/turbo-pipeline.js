...
(function(global) {
  ...
  function distributeAllKeywords(cvText, keywords, options = {}) {
    ...
    // Known company names to protect from modification - NEVER modify these lines
    ...
    // Job title patterns to protect - these lines contain job titles that MUST NOT be modified
    const JOB_TITLE_KEYWORDS = ['senior', 'lead', 'principal', 'staff', 'junior', 'associate', 'manager', 'director', 'vp', 'head', 'chief', 'engineer', 'developer', 'analyst', 'architect', 'scientist', 'designer', 'consultant', 'specialist', 'product manager', 'project manager', 'program manager', 'data analyst', 'data scientist', 'ai product manager', 'software engineer', 'solutions architect'];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // PROTECT HEADERS: Never modify lines containing company names, job titles, or pipe separators
      const lineHasPipe = trimmed.includes('|');
      const lineHasCompany = PROTECTED_COMPANIES.some(c => trimmed.toLowerCase().includes(c));
      const lineHasJobTitle = JOB_TITLE_KEYWORDS.some(t => trimmed.toLowerCase().includes(t));
      const isDateLine = /^\d{4}\s*[-–]\s*(Present|\d{4})/i.test(trimmed) || /^(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}/i.test(trimmed);
      
      // A line is a header if it has a pipe with company/job title, or is a date line, or contains company name without being a bullet
      const isBullet = /^[-•*▪▸]\s/.test(trimmed) || /^▪\s/.test(trimmed);
      const isRoleHeader = (lineHasPipe && (lineHasCompany || lineHasJobTitle)) ||
                          ...
                          isDateLine ||
                          (lineHasCompany && !isBullet); // Company name on its own line (not a bullet)

...
(function(global) {
  ...
  function enhanceExperience(experience, keywords, priorityInfo = null) {
    ...
    // Job titles to protect from modification - these are header lines, not bullet points
    const JOB_TITLE_PATTERNS = [
      /\b(senior|lead|principal|staff|junior|associate|manager|director|vp|head|chief)\b/i,
      /\b(engineer|developer|analyst|architect|scientist|designer|consultant|specialist)\b/i,
      /\b(product manager|project manager|program manager|data analyst|data scientist)\b/i,
      /\b(ai product manager|software engineer|solutions architect)\b/i
    ];
    
    // Get all bullet line indices (excluding header lines with company names or job titles)
    const bulletIndices = [];
    lines.forEach((line, idx) => {
      const trimmed = line.trim();
      // Skip header lines (contain | separator, company names, or job titles)
      const lineHasPipe = trimmed.includes('|');
      const lineHasCompany = PROTECTED_COMPANIES.some(c => trimmed.toLowerCase().includes(c));
      const lineHasJobTitle = JOB_TITLE_PATTERNS.some(p => p.test(trimmed));
      ...
      // A line is a header if it has a pipe with company/title, or is a date line
      const isHeader = (lineHasPipe && (lineHasCompany || lineHasJobTitle)) || isDateLine || (lineHasCompany && !bulletPattern.test(line));

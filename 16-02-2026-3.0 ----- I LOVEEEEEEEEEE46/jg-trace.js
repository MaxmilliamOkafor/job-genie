...
(function (global) {
  ...
  // Whole storage keys whose VALUE is secret regardless of its shape --
  // their inner keys are provider names and ids, so none of them match the
  // pattern above, and without this the tokens would be traced verbatim
  // into something written to be pasted into a bug report.
  const SECRET_STORE = /^(followup_oauth_token|enrichment_config)$/;

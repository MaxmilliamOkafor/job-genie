...
class ATSTailor {
  ...
  updateAIProviderUI() {
    ...
    const providerDisplayIcon = document.getElementById('providerDisplayIcon');
    ...
    // Update job section provider display
    if (providerDisplayIcon) {
      providerDisplayIcon.textContent = this.aiProvider === 'kimi' ? '🚀' : '🧠';
    }
    if (providerDisplayName) {
      providerDisplayName.textContent = this.aiProvider === 'kimi' ? 'Kimi K2' : 'OpenAI';
    }
  }
  ...
  async loadBaseCVFromProfile() {
    ...
    try {
      ...
      if (profile?.cv_file_path) {
        ...
        // CRITICAL FIX: Use professional_experience instead of deprecated work_experience
  ...
  bindEvents() {
    ...
    // AI Provider Toggle Buttons (Kimi K2 / OpenAI)
  ...
  async login() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn('[CHECHER] Auth error:', { status: response.status, errorCode, data });

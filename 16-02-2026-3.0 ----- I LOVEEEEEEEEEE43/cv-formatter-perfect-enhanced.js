...
(function(global) {
  ...
  const CVFormatterPerfectEnhanced = {
    ...
    normaliseDates(dateStr) {
      if (!dateStr) return '';
      return this.toYearOnly(dateStr);
    },
    ...
    parseExperience(text) {
      ...
      for (const line of lines) {
        ...
        if (trimmed.includes('|') && !trimmed.startsWith('•') && !trimmed.startsWith('-')) {
          ...
          const normalisedDates = dates ? this.toYearOnly(dates) : '';
          
          // Build titleLine: Title – MM-YYYY – MM-YYYY (using en dash with spaces)
          let titleLine = '';
...
<body>
  <div class="cv-container">
    ...
    async generatePDFWithJsPDF(htmlContent, cvData) {
      ...
      // Contact
      const cleanLocPdf = contact.location ? String(contact.location).replace(/\bopen\s+to\s+relocation\b/gi, '').replace(/^Dublin,?\s*IE$/i, '').trim() : '';

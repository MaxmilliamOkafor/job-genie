  ...
  const ResumeBuilder = {
    ...
    buildResumeWithKeywords(candidateData, keywords, options = {}) {
      ...
      const resume = {
        ...
        relevantProjects: this.buildRelevantProjectsSection(candidateData, allKeywords),
    ...
    // BUILD EXPERIENCE SECTION - SINGLE SOURCE OF TRUTH: profile.professional_experience
    // CRITICAL: NEVER modify company name, job title, or dates - only append keywords to bullet points
    // Company names and job titles MUST remain EXACTLY as provided in the profile
    ...
    buildExperienceSection(data, keywords) {
      ...
      return experience
        .map((job) => {
          // ---- HEADER: READ-ONLY from profile - NEVER modify these fields ----
          // Company name and job title MUST remain exactly as provided
          const companyLine = job.company || '';  // FIXED: Do not modify
          const title = job.title || '';  // FIXED: Do not modify
    ...
    // CRITICAL: NEVER modify project name or role title - only append keywords to bullet points
    buildRelevantProjectsSection(data, keywords) {
      ...
      return projects
        .map((project) => {
          // ---- HEADER: READ-ONLY - NEVER modify project name or role ----
          const projectName = project.name || project.company || '';  // FIXED: Do not modify
          const role = project.role || project.title || '';  // FIXED: Do not modify
          
          // Build header line: ProjectName | Role
          const headerLine = [projectName, role].filter(Boolean).join(' | ');
    ...
    generateTextVersion(resume) {
      ...
      // Relevant Projects - Include if available
      if (resume.relevantProjects) {
        sections.push('RELEVANT PROJECTS');
        sections.push(resume.relevantProjects);
        sections.push('');
      }
...
<body>
  ...
  ` : ''}
      ...
      ${resume.relevantProjects ? `
        ...
        <div class="section-content">${escapeHtml(resume.relevantProjects)}</div>
      ` : ''}

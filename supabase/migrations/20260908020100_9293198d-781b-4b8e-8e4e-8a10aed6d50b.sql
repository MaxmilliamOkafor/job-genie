CREATE OR REPLACE FUNCTION public.search_job_pool(p_terms text[] DEFAULT '{}'::text[], p_skills text[] DEFAULT '{}'::text[], p_location_terms text[] DEFAULT '{}'::text[], p_company text DEFAULT ''::text, p_workplace text[] DEFAULT '{}'::text[], p_seniority text[] DEFAULT '{}'::text[], p_employment text[] DEFAULT '{}'::text[], p_max_age_hours integer DEFAULT NULL::integer, p_require_known_date boolean DEFAULT true, p_include_unverified boolean DEFAULT true, p_sort text DEFAULT 'newest'::text, p_limit integer DEFAULT 30, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, provider text, title text, company text, location text, workplace_type text, employment_type text, department text, seniority text, url text, resolved_url text, description text, salary text, requisition_id text, posted_at timestamp with time zone, posted_at_known boolean, first_seen_at timestamp with time zone, link_status text, link_checked_at timestamp with time zone, link_http_status integer, link_note text, employer_direct boolean, risk_flags text[], duplicate_count integer, relevance real, total_count bigint)
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  RETURN QUERY
  WITH dupes AS (
    SELECT d0.dedupe_key AS dedupe_key, count(*)::integer AS n
    FROM public.job_pool d0
    WHERE d0.link_status <> 'removed'
    GROUP BY d0.dedupe_key
  ),
  filtered AS (

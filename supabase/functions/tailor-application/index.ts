...
KEYWORD PRESERVATION IN REWRITES: When rewriting a bullet from the candidate's profile, never drop a tool, technology, or skill term that the original bullet contains and the job description asks for. The rewrite may tighten the prose, but every posting-relevant term in the source bullet must survive into the rewritten bullet ("Automated the daily regulatory feed using Airflow and SQL Server" must not come back without "Airflow" when the posting asks for Airflow). If the rewrite cannot keep the term naturally, keep the candidate's original sentence unchanged instead.

...
1) CREATE RESUME with these exact sections:
   ...
   - PROFESSIONAL SUMMARY: 4-6 lines of PURE QUALIFICATIONS ONLY.
    ...
    - TARGET TITLE LINE: The line immediately after the candidate's name is a job title on its own, e.g. "Senior Backend Engineer". No pipes, no skills, no company. IT MUST BE A TITLE THE CANDIDATE HAS ACTUALLY HELD, copied from their WORK EXPERIENCE. Use the posting's title ONLY when their history already contains it; otherwise use their most recent real title, unchanged. THIS RULE USED TO SAY THE OPPOSITE -- put the posting's title there, because it is "positioning, not a claim of current employment" -- and that was wrong for a mechanical reason no amount of intent fixes: EVERY resume parser stores the line under the name as the job the candidate holds NOW. A CV reading "Business Operations Sr Analyst" above an employment block whose top entry is "Software Engineer, Meta, January 2023 - Present" produces a stored candidate record that contradicts its own history, and a reviewer holding both sees someone misrepresenting their job. On an ATS that merges candidates by email address it is worse: the stored headline changes on every application to the same employer, which is visible in the activity log. Never write a title the history does not contain, at any seniority. The extension also sets this line and now CORRECTS it when it is not a held title, so never emit it twice and never invent it.

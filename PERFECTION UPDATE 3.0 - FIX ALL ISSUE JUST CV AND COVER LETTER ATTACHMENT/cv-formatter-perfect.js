...
(function(global) {
  ...
  const CVFormatterPerfect = {
    ...
    toYearOnly(dateStr) {
      ...
      // Extract all 4-digit years as fallback
...
<body>
  <div class="cv-container">
    ...
    generateText(cvData) {
      ...
      const textContactParts = [contact.location, contact.phone, contact.email].filter(Boolean);
      if (contact.extractedJobLocation && contact.extractedJobLocation !== contact.location) textContactParts.push(contact.extractedJobLocation);
      lines.push(textContactParts.join(' | '));
    ...
    async generatePDFWithJsPDF(htmlContent, cvData) {
      ...
      // Contact
      const jsPdfContactParts = [contact.location, contact.phone, contact.email].filter(Boolean);
      if (contact.extractedJobLocation && contact.extractedJobLocation !== contact.location) jsPdfContactParts.push(contact.extractedJobLocation);
      const contactLine = jsPdfContactParts.join(' | ');

    ...
    <Card>
      ...
      <CardContent>
          ...
          {workExperience.map((exp, index) => {
            ...
            return (
              <div key={exp.id || index} className="space-y-1">
                ...
                {bullets.length > 0 && (
                  <ul
                    className="list-disc list-inside text-xs mt-2 space-y-0.5 text-gray-800"
                    style={{ fontSize: "10pt", marginLeft: "16px" }}
                  >
                    ...
                    {bullets.length > 4 && (
                      <li className="text-gray-500 italic">+{bullets.length - 4} more bullets...</li>
                    )}
                  </ul>
                )}

                {/* Spacing indicator */}
                {index < workExperience.length - 1 && <div className="h-3" aria-hidden="true" />}
              </div>
            );
          })}
        </div>
        ...
        <p className="text-xs text-muted-foreground mt-3 text-center">
          Format: <span className="font-semibold">Company</span> (bold, Line 1) + <span className="italic">Title</span>{" "}
          (italic, left) + <span>Dates</span> (right, Line 2)
        </p>
      </CardContent>
    </Card>
  );
